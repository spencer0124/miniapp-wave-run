...
describe('jump', () => {
  ...
  it('ducking in the air drops faster than falling', () => {
    ...
    let fallTicks = 0;
    let dropTicks = 0;
    while (fall.jumping) (updatePlayer(fall), fallTicks++);
    while (drop.jumping) (updatePlayer(drop), dropTicks++);
...
describe('double jump', () => {
  ...
  function doublePeak(): { peak: number; p: ReturnType<typeof createPlayer> } {
    ...
    startJump(p, 6);
    let peak = 0;
    while (p.vy > 0) (updatePlayer(p), (peak = Math.max(peak, p.y)));
    expect(startJump(p, 6)).toBe('double');
    while (p.jumping) (updatePlayer(p), (peak = Math.max(peak, p.y)));
    return { peak, p };
  }
...
describe('a run', () => {
  ...
  it('replays exactly from its seed and recorded inputs', () => {
    ...
    for (const r of original.inputs) byTick.set(r.tick, [...(byTick.get(r.tick) ?? []), r.input]);
